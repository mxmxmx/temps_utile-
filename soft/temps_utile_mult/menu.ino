...
uint16_t MULT_MODE  = 0; // left encoder: toggle mult/div and mode select
...
const uint16_t Y_OFFSET = 0x1; // y offset
  ...
  if (UI_MODE==_SCREENSAVER) {  // screen 'saver'
      ...
      if (MULT_MODE) { 
        ...
        u8g.setDefaultBackgroundColor();
          ...
          u8g.drawStr(75,  y, mult[allChannels[ch]._mult]);  
          u8g.drawStr(110, y, display_channel[ch]); 

      if (MULT_MODE)  u8g.setDefaultForegroundColor();
      ...
      for( i = 2; i < items; i++ ) {                // draw user menu, offset by 2
            ...
            if (i == ACTIVE_MENU_ITEM) {              
                u8g.drawBox(0, y-2, w, h);          // cursor bar
                u8g.setDefaultBackgroundColor();
            }
            /* print param names */
            u8g.drawStr(10, y-2, menu_strings[i+CHANNELS*allChannels[ch].mode]);
    
            /* print param values */
            print_param_values(ch, ACTIVE_MODE, i-2, x, y-2);
            u8g.setDefaultForegroundColor();    
     }              
  }
