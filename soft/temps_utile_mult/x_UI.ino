...
uint16_t button_cnt = 0; // cycle through buttons
uint16_t button_states[] = {0,0,0,0}, button_events[] ={0,0,0,0};
...
void update_ENC()  {
  ...
  if (UI_MODE == _BPM) {
    ...
    if (encoder[LEFT].change()) {
         ...
         // multiply or mode select ?
         if (!MULT_MODE && encoder[LEFT].change()) {
          
             uint8_t _channel = ACTIVE_CHANNEL;
             int16_t left_encoder_data  =  encoder[LEFT].pos();   
             LAST_UI = millis();
             
             if (left_encoder_data!= allChannels[_channel]._mult) {

                 uint8_t limit = MULT_LIMIT;

                 if      (left_encoder_data > limit)  { allChannels[_channel]._mult = limit; encoder[LEFT].setPos(limit);}
                 else if (left_encoder_data < 0) { allChannels[_channel]._mult = 0;  encoder[LEFT].setPos(0);} 
                 else    allChannels[_channel]._mult = left_encoder_data;   
                 MENU_REDRAW = 1;
           }
         }
         else if (MULT_MODE && encoder[LEFT].change())  {  // else : select mode
...
void buttons(){
   ...
   switch(x) { 
         ...
         case BOTTOM:
            state = !digitalReadFast(but_bot) && millis() - LAST_BUT > DEBOUNCE ? 1 : 0;
            break;
         case TOP:
            state = !digitalReadFast(but_top) && millis() - LAST_BUT > DEBOUNCE ? 1 : 0;
            break;
         case LEFT_SW:
            state = !digitalReadFast(butL)    && millis() - LAST_BUT > DEBOUNCE ? 1 : 0;
            break;
         case RIGHT_SW:
            state = !digitalReadFast(butR)    && millis() - LAST_BUT > DEBOUNCE ? 1 : 0;
            break;
         default:
            state = 0;
            break;
   ...
   // if pressed + ready: 
   if (state && !button_states[x]) {
              button_states[x] = PRESSED;
              button_timestamp = millis(); 
   }
}

void process_buttons(uint16_t _button) {
     ...
     switch(_b) {
          ...
          case BOTTOM:
              _pin_state = digitalReadFast(but_bot);
              break;
         case TOP:
              _pin_state = digitalReadFast(but_top);
              break;
         case LEFT_SW:
               _pin_state = digitalReadFast(butL);
               break;
         case RIGHT_SW:
               _pin_state = digitalReadFast(butR);
               break;
         default:
               _pin_state = 0;
               break;
     ...
     switch(_b_event) {
            ...
            case LEFT_SW:{
                        ...
                        switch (UI_MODE) {
                           ...
                           case _MAIN: {
                             ...
                             if (MODE_SELECTOR == _DAC) allChannels[ACTIVE_CHANNEL].param[MODE_SELECTOR][0] = 0; // if mode = DAC, no pulsewidth
                           ...
                           default: {
                             ...
                             if (MULT_MODE) encoder[LEFT].setPos(ACTIVE_MODE);
                             else encoder[LEFT].setPos(allChannels[ACTIVE_CHANNEL]._mult);
                             UI_MODE = _MAIN;
                             break;
                           }
                        }
      ...
      case HOLD: {
          ...
          switch (_b) {
             ...
             case BOTTOM: { // bottom button -> BPM page
                  UI_MODE = _BPM;
                  encoder[RIGHT].setPos(BPM);
                  break;
             }
             case TOP: {    // top button -> CV page
                  UI_MODE = _CV;
             ...
             case LEFT_SW: { // left button -> toggle mode / mult

                  uint8_t m = MULT_MODE;
                  Serial.println("ling..");
                  // 
                  if (!m) encoder[LEFT].setPos(allChannels[ACTIVE_CHANNEL].mode);        
                  else {
                        encoder[LEFT].setPos(allChannels[ACTIVE_CHANNEL]._mult);
                        MODE_SELECTOR = ACTIVE_MODE;
                  }
                  MULT_MODE = ~m & 1u;
                  break;
             }
             case RIGHT_SW: { // right button : do nothing
                  break;
             }
             default:
                  break; 
...
void sync(uint8_t _ch) {
 
   allChannels[_ch].param[DIV][3] = 0;
   allChannels[_ch].cnt = 0;
}  
